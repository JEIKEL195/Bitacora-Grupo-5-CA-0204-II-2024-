...
ggplot(data_credit, aes(x = as.factor(EDUCATION), y = LIMIT_BAL, fill = as.factor(EDUCATION))) +
  geom_boxplot() +
  labs(title = "Distribución del Límite de Crédito por Nivel Educativo", x = "Nivel Educativo", y = "Límite de Crédito") +
  theme_minimal()
```
...
ggplot(data_credit, aes(x = LIMIT_BAL, fill = as.factor(SEX), color = as.factor(EDUCATION))) +
  ...
  scale_color_manual(values = c("1" = "darkblue", "2" = "green", "3" = "orange", "4" = "purple", "5" = "gray", "6" = "black")) + 
  theme_minimal()
...
data <- data.frame(
  Encabezado = c("Nombre de Su hallazgo", 
                 ...
                 "Problemas o Posibles Desafíos", 
                 "Resumen en un párrafo"),
  Contenido = c("Relación entre la Cantidad de Crédito Otorgado con el Nivel educativo ", 
                "Un mayor nivel educativo permite ó requiere créditos de mayor valor.", 
                "No tener certeza de si el motivo del crédito es porque la persona pueda permitírselo ó lo necesite", 
                "Al utilizar el Histograma de edades por nivel educativo, con el monto de crédito aprovado, podemos observar que las personas con un nivel de Posgrado tienen mayores niveles de crédito. Sin embargo, esto puede ser dado que las personas con mayores niveles educativos puedan permitirse los niveles de crédito dado su ingreso ó más bien que las personas necesiten ese crédito para pagar sus estudios de posgrado. Estudios que generalmente son muy costosos.")
)
