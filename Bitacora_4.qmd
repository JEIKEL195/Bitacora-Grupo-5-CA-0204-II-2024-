...
ggplot(data_credit, aes(x = as.factor(default.payment.next.month), y = LIMIT_BAL, fill = as.factor(default.payment.next.month))) +
  ...
  theme_minimal()
```
Por el gráfico de la izquierda, podemos observar como a menores límite de créditos hay una concentración de las personas que caen en impago, reduciendose conforme el límite de crédito es mayor.
...
# Crear el gráfico de histograma con la variable LIMIT_BAL y clasificado por SEX y EDUCATION
ggplot(data_credit, aes(x = LIMIT_BAL, fill = as.factor(SEX), color = as.factor(EDUCATION))) +
  ...
  scale_color_manual(values = c("1" = "darkblue", "2" = "green", "3" = "orange", "4" = "purple", "5" = "gray", "6" = "black")) +  # Colores para EDUCATION
  theme_minimal()

```

Con el gráfico anterior podemos ver la relación existente entre el nivel educativo, rl género y el monto del crédito otorgado, es decir, 3 variables graficadas. 
