...
# Cambiar los nombres de las columnas en el data frame
colnames(data_credit_numerico) <- c(
  "Límite de Crédito", 
  "Edad", 
  "Monto Factura Septiembre 2005", 
  "Monto Factura Agosto 2005", 
  "Monto Factura Julio 2005", 
  "Monto Factura Junio 2005", 
  "Monto Factura Mayo 2005", 
  "Monto Factura Abril 2005", 
  "Monto Pago Septiembre 2005", 
  "Monto Pago Agosto 2005", 
  "Monto Pago Julio 2005", 
  "Monto Pago Junio 2005", 
  "Monto Pago Mayo 2005", 
  "Monto Pago Abril 2005"
)
...
```
Por último utilizamos un diagrama de violín con las variables de edad y estado de impago, para visualizar la distribución de las variables. 

```{r}
ggplot(data_credit, aes(x = as.factor(default.payment.next.month), y = AGE, fill = as.factor(default.payment.next.month))) +
  geom_violin() +
  labs(title = "Relación entre Edad y Estado de Impago", x = "Estado de Impago", y = "Edad", fill = "Impago") +
...
ggplot(data_credit, aes(x = as.factor(default.payment.next.month), y = LIMIT_BAL, fill = as.factor(default.payment.next.month))) +
  geom_violin() +
  labs(
    title = "Distribución del Límite de Crédito por Estado de Impago",
    x = "Estado de Impago",
    y = "Límite de Crédito",
    fill = "Estado de Impago"  
  ) +
  scale_fill_manual(values = c("0" = "steelblue", "1" = "lightcoral")) +
  labs(caption = "Fuente: Elaboración propia utilizando la base de datos de Kaggle") +
  theme_minimal()
...
ggplot(data_credit, aes(x = as.factor(default.payment.next.month), y = LIMIT_BAL, fill = as.factor(default.payment.next.month))) +
  geom_boxplot() +
  labs(title = "Distribución del Límite de Crédito por Estado de Impago", x = "Estado de Impago", y = "Límite de Crédito (NT dólares)", fill = "Impago") +
...
Con esto observamos que el porcentaje de los hombres que cayeron en impago, aunque es por poco, es mayor que el de las mujeres que cayeron en impago. Esto lo hicimos porque anteriormente se estaban comparando magnitudes que no se podían comparar, con los porcentajes podemos determinar que relativamente, los hombres tienden a caer más en impago que las mujeres, al menos eso podemos inferrir gracias a la evidencia de los datos. Visualicemos los gráficos una vez más, pero esta vez de manera conjunta. 


```{r}
ggplot(data_credit, aes(x = as.factor(SEX), fill = as.factor(default.payment.next.month))) +
  geom_bar(position = "dodge") +
  labs(title = "Relación entre Género y Estado de Impago", x = "Género (1=Hombre, 2=Mujer)", y = "Frecuencia", fill = "Impago") + labs(caption = "Fuente: Elaboración propia utilizando la base de datos de Kaggle") +
  scale_fill_manual(values = c("0" = "steelblue", "1" = "lightcoral")) +
  theme_minimal()
```
...
ggplot(data_credit, aes(x = as.factor(EDUCATION), fill = as.factor(default.payment.next.month))) +
  geom_bar(position = "dodge") +
  labs(
    title = "Frecuencia de Impago por Nivel Educativo",
    x = "Nivel Educativo",
    y = "Frecuencia",
    fill = "Estado de Impago"  
  ) +
...
ggplot(data_credit, aes(x = as.factor(EDUCATION), y = LIMIT_BAL, fill = as.factor(EDUCATION))) +
  ...
  labs(
    title = "Distribución del Límite de Crédito por Nivel Educativo",
    x = "Nivel Educativo",
    y = "Límite de Crédito",
  ...
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1), 
    legend.position = "top" # Comando para mover de lugar las etiquetas
  )
...
ggplot(data_credit, aes(x = LIMIT_BAL, fill = as.factor(SEX), color = as.factor(EDUCATION))) +
  geom_histogram(binwidth = 50000, alpha = 0.7, position = "identity", size = 0.3) +
  labs(
    x = "Monto del crédito otorgado (NT dólares)",
    y = "Frecuencia",
  ...
  scale_fill_manual(
    values = c("1" = "steelblue", "2" = "lightpink"),
    labels = c("1" = "Hombre", "2" = "Mujer")
  ) +
  scale_color_manual(
    values = c("1" = "darkblue", "2" = "green", "3" = "orange", "4" = "purple", "5" = "gray", "6" = "black"),
    labels = c(
      "1" = "Posgrado",
      "2" = "Universidad",
      "3" = "Secundaria",
...
data <- data.frame(
  ...
  Contenido = c("Correlación de las Variables", 
                ...
                "Al utilizar el índice de correlación de Pearson, podemos observar que existe una correlación entre las situaciones de reembolso y entre los estados de cuenta. Además, la edad podría tener una relación con el monto del préstamo.")
)
...
### Construcción de la UVE de Gowin Modificada

![UVE de Gowin Nueva](imagenes/v-gowin-2.jpeg)

## Parte de Escritura

### Escribir, escribir, escribir

#### La valoración del riesgo en el sector financiero
...
```{r fig2, fig.cap="Figura 1: Matriz de Correlación", fig.align='center'}
library(ggplot2)
library(reshape2)
...
Al utilizar el índice de correlación de Pearson, se puede observar la correlacion que existe entre las distintas variables, más en especifico entre las situaciones de reembolso y entre los estados de cuenta. Además, se observa a simple vista como la edad podría llegar a tener algún tipo de relación con el monto del préstamo.
...
De lo anterior se entiende que al momento de estimar la cantidad de personas que llegan a adquirir un crédito, en su gran mayoria lo hacen por un monto relativamente pequeño, en comparacion con la cantidad de personas que adequieren un credito de mayor valor. Lo cual puede deberse a las necesidades de las personas ó al propio riesgo que viene intrinseco al adquirir un crédito de mayor valor.
...
Al utilizar el histograma de edades por nivel educativo con la frecuencia de créditos, se puede observar que las personas con posgrados tienen una mayor frecuencia de caer en impago que las personas que no lo tienen. Esto puede ser dado que las personas con posgrado necesiten un crédito para pagar sus estudios de posgrado (los cuales generalmente son costosos) ó que puedan permitirse un crédito dados sus ingresos.
...
Ademas, las densidades que m\'as llaman nuestra atención son las de secundaria, universidad y posgrado, esto por el hecho de que ellas son las que tienen la mayor concentración de datos.Además, veamos que las densidades tienen más o menos una distribución exponencial.
...
De manera que al analizar la frecuecia de impago por sexo, podemos ver que la cantidad de mujeres en estado de impago supera a la cantidad de los hombres en ese mismo estado. Si bien no es una buena medida para todo el mundo, podemos ver que en el país de estudio, las mujeres pueden tener un peor historial crediticio que los hombres.
